...
def viterbiNbest(sentence, maxNgram, maxNumPaths, listWords, dictNgram, dictBackoff, listDefaultPaths): #Función que devuelve las listas de palabras más probables para continuar la frase inicial dada
	...
	nIni = min(len(sentenceWords), maxNgram-1) #Se obtiene la longitud de la frase inicial (como máximo el orden máximo del N-grama establecido menos 1 ya que hay que añadir la nueva palabra a evaluar)
	...
	while len(listWordsEv) == 0 and len(sentenceWords)>0: #Se buscan palabras objetivo mientra no haya ninguna y el N-grama a evaluar no esté vacío
		...
		if len(listWordsEv) == 0 and len(sentenceWords)>0: #Si la frase completa no se encuentra en el diccionario, se va acortando el N-grama eliminando las palabras iniciales
			nGramBack.append(sentenceWords[0])
